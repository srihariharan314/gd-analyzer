...
export function generateRoomCode(timeMs = Date.now()) {
...
export function generateProductionJoinUrl(roomCode) {
...
export function createHumanRoom(hostName, topic, duration = 180, maxParticipants = 6) {
    ...
    try {
        if (db) {
            const cleanKey = code.replace(/[^A-Z0-9]/g, '');
            setDoc(doc(db, "rooms", cleanKey), room).catch(err => {
                console.warn("Firestore room save note:", err);
            });
        }
    } catch (err) {
        console.warn("Firestore room save error:", err);
    }
...
export async function getRoomState(roomCode) {
    ...
    if (!room) {
        ...
        // Active room reconstructed for joining peer
        room = {
    ...
    // Check expiration on found room
...
export async function joinHumanRoom(roomCode, participantName) {
    ...
    if (!existing) {
        ...
        try {
            if (db) {
                const cleanKey = room.roomCode.replace(/[^A-Z0-9]/g, '');
                setDoc(doc(db, "rooms", cleanKey), room).catch(err => {
                    console.warn("Firestore participant update note:", err);
                });
            }
        } catch (e) {}
    }
